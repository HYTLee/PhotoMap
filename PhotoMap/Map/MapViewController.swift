...
class MapViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @objc func openCamera(){
        ...
        let openCameraButton = UIAlertAction(title: "Open Camera", style: .default) { (_) in
            self.showImagePickerController(sourceType: .camera, sourceOfLocation: .curentLocation)
        }
        let openGaleryButton = UIAlertAction(title: "Open Galery", style: .default) { [self] (_) in
            showImagePickerController(sourceType: .photoLibrary, sourceOfLocation: .curentLocation)
        }
